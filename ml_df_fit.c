...
void get_grid_size(double min_lat){
  int j,jp;
...
void make_grid(double min_lat){
  ...
  int nlon,nlon_ng,nlon_m,nlon_p;
  int nlat=(max_lat-min_lat)/dlat;
  int jlat,jlon,jlat_ng,jlon_ng,jgrid;
...
void make_div_ar(){
  int igr,iedg,inr0,inr1,inr2;
  double fr0,fr1,fr2,chk;
  int idv;
  int flg;
  int ind;
  double dx,dy,dlat_l,dlon_l;
  double dlon0;
  FILE *divfile;
  ...
  for( igr=nedge; igr<ngrid; igr++){
    ...
    /* inr0=neighbors[igr].left[0]; */
    /* inr1=neighbors[igr].left[1]; */
...
void regrid_ml(MLDstr *mlDstr, GridMLDstr *rgMLDstr){
  ...
  for( j=0; j<ngrid; j++ ){
    ...
    ml_lat_indx=(int)((lat-ML_LAT_0)/ML_DLAT);
...
void get_pos_ar(struct RadarSite *site, struct RadarParm *prm, struct RadarPos *rdrpos){
  ...
  for( bm=0; bm<site->maxbeam; bm++) for( rn=0; rn<=prm->nrang; rn++){
      ...
      /* RPosMag(0,bm,rn,site,frang,rsep, */
      /* 	      site->recrise,0,&rho,&lat,&lon,chis,0); */
...
void map_pos_to_grid(struct RadarPos rdrpos, struct RadarMap *rmap){
  ...
  for( ib=0; ib<MAX_BEAM; ib++)for( ir=0; ir<MAX_RANGE; ir++){
      mlat=rdrpos.lat[ib][ir];
      mlon=rdrpos.lon[ib][ir];
      rmap->cell[ib][ir]=-1;
      for(ig=0; ig<ngrid; ig++)if(cell_in_out(mlat,mlon,grid[ig]))rmap->cell[ib][ir]=ig;
    }
}
...
int main(int argc, char *argv[]){
  ...
  */
  ...
  struct tm *t_start;
  struct tm *t_end; 
  time_t fit_time, ssec, esec, istart, iend;
  char *envstr;
  ...
  while( fit_time<esec )
	...
	if( rptrs[jj]!=NULL ){
	  ...
	  while( ((ftime(prm)-fit_time)<avg_ival/2) && (try_count<count_lim)){
		...
		nrangl[jj]=prm->nrang;
	      ...
	      if( fabs(ftime(prm)-fit_time) < (double)avg_ival/2){
		yr=prm->time.yr;
		mo=prm->time.mo;
		dy=prm->time.dy;
		hr=prm->time.hr;
	...
	for( jc=0; jc<nbms; jc++ )for( jr=1; jr<prm->nrang; jr++){
		...
		filt_cnt[jc][jr]>0 && rpos[jj].lat[jc][jr] < MAX_DATA_LAT){	
